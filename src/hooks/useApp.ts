...
export const useApp = () => {
  ...
  // Toggle window visibility (called from Rust when shortcut fires).
  // Works on all platforms now — the Rust side emits this event whenever
  // it toggles the overlay's hidden state.
  useEffect(() => {
    const unlistenPromise = listen<boolean>(
      "toggle-window-visibility",
      (event) => {
        if (typeof event.payload === "boolean") {
          // payload = true means hidden, false means shown
          setIsHidden(event.payload);

          // If the window just became hidden, force-close any open popovers
          // so the UI doesn't stay "stuck open" in memory while the window
          // is invisible.
          if (event.payload) {
