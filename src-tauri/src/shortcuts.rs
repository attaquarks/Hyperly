...
pub fn start_move_window<R: Runtime>(app: &AppHandle<R>, direction: &str) {
    // License gating removed: 4-way window movement is always available.
...
fn handle_toggle_window<R: Runtime>(app: &AppHandle<R>) {
    ...
    //   - Track our own "hidden" flag in Rust state (the OS visibility check
...
pub fn update_shortcuts<R: Runtime>(
    ...
    // License gating removed: all shortcuts (including move_window) are
